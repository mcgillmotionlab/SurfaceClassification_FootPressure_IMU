...
\Title{\highlighting{Outdoor} %MDPI: The title is different from the one submitted online at susy.mdpi.com (Walking classification based on inertial measurement unit and foot pressure sensor data). Please confirm which is correct.
...
\address{%
$^{1}$ \quad Department of Kinesiology and Physical Education, McGill University, Montreal, \hl{QC H3A 0G4,} %MDPI: We added postcode. Please confirm. Same below.
...
\begin{figure}[H]
%\centering
\hspace{-3em}\includegraphics[width=\textwidth, height=0.25\textheight, keepaspectratio]{figures/lower limb config.png}
...
\begin{figure}[H]
    ...
    \centering\includegraphics[width=18cm]{figures/learning-flow-chart.png}
    \end{adjustwidth}
...
\midrule
\multirow{6}{*}{\textbf{\hl{ML} %MDPI: Please confirm if the bold formatting is necessary; if not, please remove it. The following highlights are the same.
}} 
...
\noindent{\footnotesize{Mean (standard deviation) shown. See main text for details on sensor configurations. Best experimental model shown in bold. IMU lower-limbs model shown for comparison purposes. Significant differences are indicated with superscript letters, see footnote for details. \hl{\textsuperscript{a}}: %MDPI: We have changed it to superscript. Please confirm.
...
%\centering
\caption{Classification performance for different window lengths. Values in parentheses represent the standard deviation across ten trials with different random seeds.}
\begin{tabular}{lccccc}
\toprule
\textbf{Model \textsuperscript{1}} & \textbf{Window Length} &  \textbf{Accuracy} & \textbf{F1-Score} & \textbf{Sensitivity} & \textbf{Specificity} \\
\midrule
\multirow{3}{*}{ML} 
  & 1 s & 0.81 (0.04) & 0.79 (0.04) \textsuperscript{a} & 0.78 (0.03) & 0.93 (0.01) \\
...
\noindent{\footnotesize{\textsuperscript{1}: Pelvis + pressure configuration. \textsuperscript{a}: F1-score for that segment length is significantly different from the 2-s one.}}
...
\funding{\hl{This research was funded} %MDPI: Information regarding the funder and the funding number should be provided. Please check the accuracy of funding data and any other information carefully. Any updates after publication should be carefully considered.
 by the Fonds de Recherche du Qu\'ebec (FRQ) Audace International Qu\'ebec--Luxembourg and by the Fonds National de la Recherche Luxembourg (FNR). P.C.D. acknowledges support
...
\midrule
\multicolumn{8}{c}{\textbf{\hl{IMU-only} %MDPI: Please confirm if the bold formatting is necessary; if not, please remove it. The following highlights are the same.
}}\\
\midrule
...
\textbf{Paper}	& \textbf{Participants}	& \textbf{Surfaces (Categorized)} & \textbf{Instrumentation} & \textbf{Model(s)} & \textbf{Evaluation} & \textbf{Metrics} & \textbf{Notes/Setting}\\
\midrule
\multicolumn{8}{c}{\textbf{\hl{Multimodal (EMG/motion capture/insoles)}}}\\
\midrule
...
\begin{figure}[H]
 ...
 knee angles, normalized to 100\% of the gait cycle, available from the processed IMUs to gauge the success of the gait segmentation strategy.}
...
\noindent{\footnotesize{\textsuperscript{1}: Pelvis + pressure configuration.}}
